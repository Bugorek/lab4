...
void Game::reset()
{
    ...
    if (this->gameMode != MT_SHEEP)
        runMinMax((gameMode == MT_SHEEP) ? MT_WOLF : MT_SHEEP, 0, -EPIC_BIG_VALUE, +EPIC_BIG_VALUE);
...
bool Game::isGameOver(MonsterType& winner)
{
    ...
    if (!canMove || this->sheep.y() == 0)
        winner = MT_SHEEP;

    canMove = false;
    for (int i = 0; i < 4; i++)
        if (canMoveToPosition(0, this->sheep + this->possibleMoves[i]))
            canMove = true;
...
bool Game::moveSelectedMonsterToPosition(const QPoint &pos)
{
    ...
    runMinMax(this->gameMode == MT_SHEEP ? MT_WOLF : MT_SHEEP, 0, -EPIC_BIG_VALUE, +EPIC_BIG_VALUE);
    this->playersTurn = !this->playersTurn;
...
void Game::prepareMap()
{
    ...
    this->map[this->sheep.y()][this->sheep.x()] = SHEEP;
    for (int i = 0; i < 4; i++)
...
int Game::getHeuristicEvaluation()
{
    ...
    this->searchWay.enqueue(this->sheep);
...
void Game::temporaryMonsterMovement(int monsterIndex, int x, int y)
{
    if (monsterIndex == 0)
    {
        this->map[this->sheep.y()][this->sheep.x()] = EMPTY;
        this->map[this->sheep.y() + y][this->sheep.x() + x] = SHEEP;
        this->sheep += QPoint(x, y);
    } else
    {
...
int Game::runMinMax(MonsterType monster, int recursiveLevel, int alpha, int beta)
{
    ...
    for (int i = (isWolf ? 0 : 8); i < (isWolf ? 8 : 12); i++)
    {
        ...
        QPoint curMonsterPos = curMonster == 0 ? this->sheep : this->wolfs[curMonster - 1];
        ...
        if (canMove(curMonsterPos + curMove))
        {
            ...
            test = runMinMax(isWolf ? MT_SHEEP : MT_WOLF, recursiveLevel + 1, alpha, beta);

            //... и восстанавливаем исходное состояние
            temporaryMonsterMovement(curMonster, -curMove);

            //если он лучше всех, что были до этого - запомним, что он лучший
            if ((test > MinMax && monster == MT_WOLF) || (test <= MinMax && monster == MT_SHEEP) || (bestMove == NOT_INITIALIZED))
            {
                MinMax = test;
                bestMove = i;
            }
    ...
    if (recursiveLevel == 0 && bestMove != NOT_INITIALIZED)
    {
        if (monster == MT_WOLF)
            ...
            this->sheep += this->possibleMoves[bestMove % 4];
    }
