...
{
    ui->setupUi(this);
    this->setWindowTitle("Sheep&Wolfs");
...
void View::pbPlayClicked()
{
    ...
    this->game->setPlayMode(Game::MT_SHEEP);
    this->game->setAILevel(1);
    this->game->reset();
}
...
{    
    ...
    for (int i = 0; i < this->game->getMonsterCount(); i++)
        ...
        {
            p.setBrush(QBrush(QColor(this->game->getMonsterType(i) == Game::MT_WOLF ? 200 : 40, 40, this->game->getMonsterType(i) == Game::MT_SHEEP ? 200 : 40)));
    ...
    if (game->getSelectedMonsterIndex() >= 0)
    {
        ...
        p.setBrush(QBrush(QColor(this->game->getMonsterType(this->game->getSelectedMonsterIndex()) == Game::MT_WOLF ? 255 : 40, 40,
                                 this->game->getMonsterType(this->game->getSelectedMonsterIndex()) == Game::MT_SHEEP ? 255 : 40)));
